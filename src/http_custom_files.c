...
static const char INDEX_HTML[] =
  ...
  "<label>Decode <select id='decode'><option value='plain'>plain</option><option value='ansi'>ansi colors</option></select></label>"
  ...
  "</div>"
  "<div id='shell'><div id='term' class='pane' tabindex='0'></div><div id='hex' class='pane'></div></div>"
  "<script>"
  ...
  " }else{const b=new Uint8Array(e.data);queueHex(b);const txt=dec.decode(b);if(decodeMode.value==='ansi')appendAnsi(txt);else appendTerm(txt);}"
  "};"
  "applyBtn.onclick=()=>{"
  ...
  "document.getElementById('clear').onclick=()=>{term.textContent='';hex.textContent='';hexOffset=0;lineBuf='';pendingHex=[];if(hexTimer){clearTimeout(hexTimer);hexTimer=null;}sgr.bold=false;sgr.fg='';sgr.bg='';keepCursor();};"
